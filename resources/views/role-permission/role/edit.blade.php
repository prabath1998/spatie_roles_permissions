      ...
      <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
          <div class="p-6 bg-white border-b border-gray-200">
            <h5 class="text-2xl font-bold mb-4">Edit Role <span class="text-red-500">{{ $role->name }}</span></h5>
            <a href="{{ url('roles') }}"
          ...
          <div class="p-6 bg-white">
            <form action="{{ url('roles/' . $role->id) }}" method="POST" class="space-y-6">
              @csrf
              @method('PUT')

              <div>
                <label for="name" class="block text-gray-700 font-bold mb-2">role Name</label>
                <input type="text" name="name" value="{{ $role->name }}" id="name" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" />
              </div>
